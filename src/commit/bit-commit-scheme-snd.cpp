...
BitCommitSender::BitCommitSender(CommonTools& common_tools, uint8_t seeds0[], uint8_t seeds1[]) :
  BitCommitScheme(common_tools),
  seeds0(seeds0),
  seeds1(seeds1) {
}

void BitCommitSender::Commit(uint32_t num_commits, BYTEArrayVector& commit_shares0_res, BYTEArrayVector& commit_shares1_res, uint32_t current_counter) {
  ...
  ExpandAndTranspose(matrices0, matrices1, commit_shares0, commit_shares1, current_counter);
  CheckbitCorrection(commit_shares0, commit_shares1);
  ConsistencyCheck(matrices0, matrices1, commit_shares0, commit_shares1);

  for (int i = 0; i < num_commits; ++i) {
    std::copy(commit_shares0[i], commit_shares0[i] + BIT_CODEWORD_BYTES, commit_shares0_res[i]);
    std::copy(commit_shares1[i], commit_shares1[i] + BIT_CODEWORD_BYTES, commit_shares1_res[i]);
  }
}

void BitCommitSender::ExpandAndTranspose(std::vector<std::unique_ptr<uint8_t[]>>& matrices0, std::vector<std::unique_ptr<uint8_t[]>>& matrices1, std::vector<uint8_t*>& commit_shares0, std::vector<uint8_t*>& commit_shares1, uint32_t current_counter) {
  ...
  for (int i = 0; i < BIT_CODEWORD_BITS; ++i) {
    ...
    seed0 += current_counter;
    seed1 += current_counter;
    ...
    for (int j = 0; j < num_blocks; ++j) {
      rnd0.GenRnd(matrices0[j].get() + i * col_dim_bytes, col_dim_bytes);
      rnd1.GenRnd(matrices1[j].get() + i * col_dim_bytes, col_dim_bytes);
    }
  }
  ...
  for (int j = 0; j < num_blocks; ++j) {
    transpose_64_128(matrices0[j].get(), col_blocks);
    transpose_64_128(matrices1[j].get(), col_blocks);
...
void BitCommitSender::CheckbitCorrection(std::vector<uint8_t*>& commit_shares0, std::vector<uint8_t*>& commit_shares1) {

  //Buffers
  BYTEArrayVector checkbit_corrections_buf(num_commits_produced, BIT_CODEWORD_BYTES);

  uint8_t values_buffer[BIT_CODEWORD_BYTES];
  uint8_t value;
  //Compute all the corrections and send to receiver

  for (int j = 0; j < num_commits_produced; ++j) {
    XOR_BitCodeWords(values_buffer, commit_shares0[j], commit_shares1[j]);
    value = GetBit(0, values_buffer);

    Encode(value, checkbit_corrections_buf[j]);

    XOR_BitCodeWords(commit_shares1[j], commit_shares0[j], checkbit_corrections_buf[j]);
    XOR_BitCodeWords(checkbit_corrections_buf[j], values_buffer);
  }

  //Needs to be SendBlocking, else code hangs. Cannot explain why as everywhere else Send works fine.
  common_tools.chan.SendBlocking(checkbit_corrections_buf.GetArray(), checkbit_corrections_buf.size);
}

void BitCommitSender::ConsistencyCheck(std::vector<std::unique_ptr<uint8_t[]>>& matrices0, std::vector<std::unique_ptr<uint8_t[]>>& matrices1, std::vector<uint8_t*>& commit_shares0, std::vector<uint8_t*>& commit_shares1) {
  ...
  uint8_t final_result0[2 * BIT_CODEWORD_BYTES * 2 * SSEC];
  uint8_t* final_result1 = final_result0 + BIT_CODEWORD_BYTES * 2 * SSEC;
  ...
  __m128i res_tmps[4][BIT_CODEWORD_BITS];
  __m128i res_totals[2][BIT_CODEWORD_BITS];
  for (int i = 0; i < BIT_CODEWORD_BITS; ++i) {
  ...
  std::unique_ptr<uint8_t[]> matrices_tmp0(std::make_unique<uint8_t[]>(2 * transpose_matrix_size));
  uint8_t* matrices_tmp1 = matrices_tmp0.get() + transpose_matrix_size;

  //Receive challenge seed from receiver and load initial challenge alpha
  uint8_t alpha_seed[CSEC_BYTES];
  common_tools.chan.ReceiveBlocking(alpha_seed, CSEC_BYTES);
  ...
  for (int j = 0; j < num_blocks; ++j) {
    ...
    transpose_128_64(matrices_tmp0.get(), col_blocks);
    transpose_128_64(matrices_tmp1, col_blocks);
  ...
  common_tools.chan.Send(final_result0, 2 * BIT_CODEWORD_BITS * 2 * SSEC_BYTES);
}

void BitCommitSender::BatchDecommit(uint8_t decommit_shares0[], uint8_t decommit_shares1[], int num_values) {
  ...
  uint8_t final_result0[2 * BIT_CODEWORD_BYTES * SSEC];
  uint8_t* final_result1 = final_result0 + BIT_CODEWORD_BYTES * SSEC;
  ...
  for (int j = 0; j < num_check_blocks; ++j) {
    ...
    for (int i = 0; i < col_dim; ++i) {
      ...
      if (num_check_index < num_values) {
        std::copy(decommit_shares0 + num_check_index * BIT_CODEWORD_BYTES, decommit_shares0 + num_check_index * BIT_CODEWORD_BYTES + BIT_CODEWORD_BYTES, matrices_tmp0.get() + i * row_dim_bytes);
        std::copy(decommit_shares1 + num_check_index * BIT_CODEWORD_BYTES, decommit_shares1 + num_check_index * BIT_CODEWORD_BYTES + BIT_CODEWORD_BYTES, matrices_tmp1 + i * row_dim_bytes);
      }
      else { //this pads the last block with 0 rows
        std::fill(matrices_tmp0.get() + i * row_dim_bytes, matrices_tmp0.get() + i * row_dim_bytes + BIT_CODEWORD_BYTES, 0);
        std::fill(matrices_tmp1 + i * row_dim_bytes, matrices_tmp1 + i * row_dim_bytes + BIT_CODEWORD_BYTES, 0);
      }
    }
  ...
  common_tools.chan.Send(final_result0, 2 * BIT_CODEWORD_BYTES * SSEC);
}