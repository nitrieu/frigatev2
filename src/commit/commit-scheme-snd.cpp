...
CommitSender::CommitSender(CommonTools& common_tools, uint8_t seeds0[], uint8_t seeds1[]) :
  CommitScheme(common_tools),
  seeds0(seeds0),
  seeds1(seeds1) {
}

void CommitSender::Commit(uint32_t num_commits, BYTEArrayVector& commit_shares0_res, BYTEArrayVector& commit_shares1_res, uint32_t current_counter, uint32_t set_lsb_start_idx) {
  ...
  CheckbitCorrection(commit_shares0, commit_shares1, set_lsb_start_idx, num_commits);
  ConsistencyCheck(matrices0, matrices1, commit_shares0, commit_shares1);

  for (int i = 0; i < num_commits; ++i) {
    std::copy(commit_shares0[i], commit_shares0[i] + CODEWORD_BYTES, commit_shares0_res[i]);
    std::copy(commit_shares1[i], commit_shares1[i] + CODEWORD_BYTES, commit_shares1_res[i]);
  }
}

void CommitSender::ExpandAndTranspose(std::vector<std::unique_ptr<uint8_t[]>>& matrices0, std::vector<std::unique_ptr<uint8_t[]>>& matrices1, std::vector<uint8_t*>& commit_shares0, std::vector<uint8_t*>& commit_shares1, uint32_t current_counter) {
...
void CommitSender::CheckbitCorrection(std::vector<uint8_t*>& commit_shares0, std::vector<uint8_t*>& commit_shares1, uint32_t set_lsb_start_idx, uint32_t num_commits) {

  //Is used to fix lsb of specific committed values in a range starting from set_lsb_start_idx. Default sets bit to 0
  // std::vector<uint8_t> flipped_values(num_commits);
  BYTEArrayVector flipped_values(BITS_TO_BYTES(num_commits), 1);

  uint8_t commit_value[CSEC_BYTES];
  uint8_t delta_flip_table[] = {REVERSE_BYTE_ORDER[1], 0};
  for (int i = 0; i < num_commits; ++i) {
    XOR_128(commit_value, commit_shares0[i], commit_shares1[i]);
    SetBit(i, GetLSB(commit_value), flipped_values.GetArray());
    if (i < set_lsb_start_idx) {
      commit_shares1[i][CSEC_BYTES - 1] ^= REVERSE_BYTE_ORDER[GetLSB(commit_value)];
    } else {
      commit_shares1[i][CSEC_BYTES - 1] ^= delta_flip_table[GetLSB(commit_value)];
    }
  }

  common_tools.chan.Send(flipped_values.GetArray(), flipped_values.size);
  ...
  common_tools.chan.SendBlocking(checkbit_corrections_buf.get(), num_commits_produced * BCH_BYTES);
}

void CommitSender::ConsistencyCheck(std::vector<std::unique_ptr<uint8_t[]>>& matrices0, std::vector<std::unique_ptr<uint8_t[]>>& matrices1, std::vector<uint8_t*>& commit_shares0, std::vector<uint8_t*>& commit_shares1) {
  ...
  common_tools.chan.Send(final_result0, 2 * CODEWORD_BYTES * 2 * SSEC);
}
...
void CommitSender::BatchDecommit(uint8_t decommit_shares0[], uint8_t decommit_shares1[], int num_values) {
  ...
  common_tools.chan.Send(final_result0, 2 * CODEWORD_BYTES * SSEC);
}
...
//   common_tools.chan.Send(tmp_values.get(), num_values * CSEC_BYTES);
// }