...
CommitReceiver::CommitReceiver(CommonTools& common_tools, uint8_t seeds[], uint8_t choices[]) : CommitScheme(common_tools), seeds(seeds), choices(choices) { }

bool CommitReceiver::Commit(uint32_t num_commits, BYTEArrayVector& commit_shares_res, uint32_t current_counter, uint32_t set_lsb_start_idx) {
  ...
  CheckbitCorrection(commit_shares, set_lsb_start_idx, num_commits);
  if (ConsistencyCheck(matrices, commit_shares)) {
    for (int i = 0; i < num_commits; ++i) {
      std::copy(commit_shares[i], commit_shares[i] + CODEWORD_BYTES, commit_shares_res[i]);
    }
    return true;
...
void CommitReceiver::ExpandAndTranspose(std::vector<std::unique_ptr<uint8_t[]>>& matrices, std::vector<uint8_t*>& commit_shares, uint32_t current_counter) {
...
void CommitReceiver::CheckbitCorrection(std::vector<uint8_t*>& commit_shares, uint32_t set_lsb_start_idx, uint32_t num_commits) {
  ...
  common_tools.chan.ReceiveBlocking(lsb_corrections.GetArray(), lsb_corrections.size);

  uint8_t delta_flip_table[] = {REVERSE_BYTE_ORDER[1], 0};
  for (int i = 0; i < num_commits; ++i) {
      if (i < set_lsb_start_idx) {
        commit_shares[i][CSEC_BYTES - 1] ^= bit_to_byte[GetLSB(choices)] & REVERSE_BYTE_ORDER[GetBit(i, lsb_corrections.GetArray())];
      } else {
        commit_shares[i][CSEC_BYTES - 1] ^= bit_to_byte[GetLSB(choices)] & delta_flip_table[GetBit(i, lsb_corrections.GetArray())];
      }
  }
  ...
  common_tools.chan.ReceiveBlocking(checkbit_corrections_buf.get(), num_commits_produced * BCH_BYTES);
...
bool CommitReceiver::ConsistencyCheck(std::vector<std::unique_ptr<uint8_t[]>>& matrices, std::vector<uint8_t*>& commit_shares) {
  ...
  // // ///////////////////////TEST//////////////////////////////////
  // auto t = GET_TIME();
  // uint8_t* V = new uint8_t[BITS_TO_BYTES(common_tools.num_commits * CSEC)];
  // common_tools.rnd.GenRnd(V, BITS_TO_BYTES(common_tools.num_commits * CSEC));
  // uint8_t* linear_combination = new uint8_t[CODEWORD_BYTES*CSEC];
  // for (int l = 0; l < common_tools.num_commits; l++) {
  ...
  common_tools.chan.ReceiveBlocking(decommit_shares0, 2 * CODEWORD_BITS * 2 * SSEC_BYTES);
...
bool CommitReceiver::BatchDecommit(uint8_t computed_shares[], int num_values, uint8_t values[]) {
  ...
  common_tools.chan.ReceiveBlocking(decommit_shares0, 2 * CODEWORD_BYTES * SSEC);
...
//   common_tools.chan.ReceiveBlocking(chosen_commit_values.get(), num_values * CSEC_BYTES);
// }
...
//   common_tools.chan.ReceiveBlocking(decommit_shares0.get(), num_values * (CODEWORD_BYTES + CSEC_BYTES));
...
bool CommitReceiver::VerifyTransposedDecommits(uint8_t decommit_shares0[], uint8_t decommit_shares1[], uint8_t computed_shares[], int num_values) {
  ...
  transpose_320_128(matrix0.get(), matrix1, 1);
