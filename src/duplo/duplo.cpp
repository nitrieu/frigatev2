...
Duplo::Duplo(CommonTools& common_tools, uint32_t num_max_parallel_execs) :
  ...
  inp_bucket_circuit.num_non_free_gates = 0;
  ...
  GenExecTools(num_max_parallel_execs);
}

void Duplo::WeightedRandomString(uint8_t res[], int weight, int res_length, PRNG& rnd, bool negate_probability) {
...
void Duplo::ComputeIndices(uint32_t num_circuits, Circuit& circuit, uint32_t& num_inp_keys, uint32_t& num_out_keys, uint32_t& num_deltas, uint32_t& num_commit_keys, uint32_t& num_base_keys, uint32_t& input_keys_idx, uint32_t& output_keys_idx, uint32_t& deltas_idx) {
...
void Duplo::FindBestSingleParams(uint32_t num_buckets, uint32_t& res_bucket, long double& check_val, bool& check_val_negate) {

  uint32_t upper_bucket_bound = 50;
...
void Duplo::MaxSingleProb(int b, uint32_t num_buckets, mpf_class& curr_p, mpf_class& max_prob) {
...
void Duplo::FindBestMajorityParams(uint32_t num_buckets, uint32_t& res_bucket, long double& check_val, bool& check_val_negate, uint32_t catch_reciproc_prob) {

  uint32_t upper_bucket_bound = 100;
  uint32_t lower_check_p_bound = 10;
  uint32_t lower_check_p_inv_bound = 2;
  uint32_t num_inv_iterations = 0;
  mpz_class target_prob_inv;
...
void Duplo::MaxProbMajority(int b, uint32_t num_buckets, mpf_class& curr_p, mpf_class& max_prob, uint32_t catch_reciproc_prob) {
...
void Duplo::ComputeCheckFraction(int check_frac, int num_items, float & slack_frac, float & final_rep_frac, bool negate_check_probability) {
...
void Duplo::GenExecTools(uint32_t num_parallel_execs) {
...
uint32_t Duplo::GetNumInpWires(ComposedCircuit& composed_circuit) {

  uint32_t num_total_inp_wires = 0;
  for (int l = 0; l < composed_circuit.num_inp_circuits; ++l) {
    ...
    num_total_inp_wires += circuit.num_inp_wires;
  }

  return num_total_inp_wires;
}

uint32_t Duplo::GetNumOutWires(ComposedCircuit& composed_circuit, OutType out_type) {

  uint32_t num_circuits = composed_circuit.circuits.size();
  uint32_t num_total_out_wires = 0;
  for (int l = num_circuits - composed_circuit.num_out_circuits; l < num_circuits; ++l) {
    std::string component_name(std::get<0>(composed_circuit.circuits[l]));
    Circuit& circuit = string_to_circuit_map[component_name];
    
    if (out_type == CONST_OUT) {
      num_total_out_wires += circuit.num_const_out_wires;  
    } else if (out_type == EVAL_OUT) {
      num_total_out_wires += circuit.num_eval_out_wires;
    } else if (out_type == ALL_OUT) {
      num_total_out_wires += circuit.num_out_wires;
    }
  }

  return num_total_out_wires;
}

uint32_t Duplo::GetNumTotalOutWires(ComposedCircuit& composed_circuit) {

  uint32_t num_total_wires = GetNumInpWires(composed_circuit);
  for (int l = 0; l < composed_circuit.circuits.size(); ++l) {
    std::string component_name(std::get<0>(composed_circuit.circuits[l]));
    Circuit& circuit = string_to_circuit_map[component_name];
    num_total_wires += circuit.num_out_wires;
  }

  return num_total_wires;
}