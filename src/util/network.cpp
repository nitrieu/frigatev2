...
Network::Network(const char* m_nAddr, USHORT m_nPort, uint32_t role) : m_nAddr(m_nAddr) {
  m_vSocket = new CSocket();

  sndthread = new SndThread(m_vSocket);
  rcvthread = new RcvThread(m_vSocket);

  // this->m_nAddr = m_nAddr;
...
#ifdef LOUD
  std::cout << "Connecting to party " << !role << ": " << m_nAddr << ", " << m_nPort << std::endl;
#endif
  for (int k = 0; k >= 0 ; k--)
  {
...
#ifdef LOUD
        std::cout << " (" << !role << ") (" << k << ") connected" << std::endl;
#endif
        if (k == 0)
        {
...
connect_failure:
  std::cout << " (" << !role << ") connection failed" << std::endl;
  return FALSE;
}
