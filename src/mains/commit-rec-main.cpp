...
int main(int argc, const char* argv[]) {
  ...
  opt.example = "Commitrec -n 10000 -e 4 -ip 10.11.100.216 -p 28001\n\n";
  ...
  std::unique_ptr<uint8_t[]> dot_response(std::make_unique<uint8_t[]>(params.num_OT * CSEC_BYTES));
  std::unique_ptr<uint8_t[]> dot_choices(std::make_unique<uint8_t[]>(BITS_TO_BYTES(params.num_OT)));
  ...
  for (int i = 0; i < CODEWORD_BITS; ++i) {
    if (GetBit(rot_start_pos + i, dot_choices.get())) {
  ...
  for (int i = 0; i < CODEWORD_BITS; ++i) {
    params.crypt.hash(rot_seeds.get() + i * CSEC_BYTES, CSEC_BYTES, dot_response.get() + (rot_start_pos + i) * CSEC_BYTES, CSEC_BYTES);
  }
  ...
  std::cout << "Commit us: " << (double) (commit_time_nano + base_ot_time_nano) / params.num_commits / 1000 << std::endl;
