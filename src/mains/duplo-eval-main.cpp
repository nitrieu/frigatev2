...
int main(int argc, const char* argv[]) {
  ...
  circuit_name = "eval_" + circuit_name;
...
/*
   ...
   for (int i = 0; i < circuit.num_eval_inp_wires; ++i) {
     if (GetBitReversed(i, buffer[0]  + BITS_TO_BYTES(circuit.num_eval_inp_wires))) {
...
*/
  ...
  duplo_eval.PreprocessComponentType(circuit_name, circuit, num_iters, num_execs_components);
  ...
  duplo_eval.DecodeKeys(pairs, output_keys, outputs, num_execs_online);
