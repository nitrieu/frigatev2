...
int main(int argc, const char* argv[]) {
    ...
    circuit_name_id = circuit_name + "_id";
    circuit_name_tag = circuit_name + "_tag";
    circuit_name_aes = circuit_name + "_aes";
...
//    Setup the main common_tools object
    CommonTools common_tools(constant_seeds[1], ip_address, (uint16_t) port, 1, context, GLOBAL_PARAMS_CHAN);
    ...
    DuploEvaluator duplo_eval(common_tools, max_num_parallel_execs);
...
//    Run Preprocessing phase
    ...
    duplo_eval.PreprocessComponentType(circuit_name_id, circuit_id, 1, num_execs_components);
    std::cout << "ID PREPROCESSED\n";
    duplo_eval.PreprocessComponentType(circuit_name_tag, circuit_tag, 1, num_execs_components);
    std::cout << "TAG PREPROCESSE\n";
    duplo_eval.PreprocessComponentType(circuit_name_aes, circuit_aes, num_iters, num_execs_components);
    std::cout << "AES PREPROCESSE\n";
...
//    Soldering of the tree circuits
    ...
    duplo_eval.SolderGarbledComponents(circuit_name_composed, composed_circuit, num_execs_components);
...
//    Run Auth Preprocessing phase
    ...
    for (int i = 0; i < composed_circuit.num_inp_circuits; ++i) {
        inputs.emplace_back(std::vector<uint8_t>(BITS_TO_BYTES(circuit_id.num_eval_inp_wires)));
    }
...
//    Evaluating components
    ...
    duplo_eval.EvalComposedComponents(circuit_name_composed, inputs, eval_output_keys, num_execs_online);
...
//    Decoding keys
    ...
    PrintBin(eval_outputs[0].data(), 128);
    PrintBin(eval_outputs[1].data(), 128);

    std::vector<uint8_t> evals0(circuit_aes.num_wires);
    for (int i = 0; i < circuit_aes.num_gates; ++i) {
        Gate& g = circuit_aes.gates[i];
        if (g.type == NOT) {
    ...
    uint8_t res[CSEC_BYTES];
    for (int i = 0; i < circuit_aes.num_out_wires; ++i){
        if(evals0[circuit_aes.out_wires_start + i]) {
            SetBit(i, 1, res);
        } else {
            SetBit(i, 0, res);
        }
