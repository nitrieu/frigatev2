...
int main(int argc, const char* argv[]) {
  ...
  std::unique_ptr<uint8_t[]> eval_input(std::make_unique<uint8_t[]>(BITS_TO_BYTES(circuit.num_eval_inp_wires)));
  ...
  if (extra_execs < 0) {
    ...
    for (int i = 0; i < extra_execs; ++i) {
      ...
      tiny_eval.commit_recs.emplace_back(std::make_unique<CommitReceiver>(*tiny_eval.thread_params_vec[params.num_execs + i], tiny_eval.rot_seeds.get(), tiny_eval.rot_choices));
    }
  }
  ...
  for (int i = 0; i < circuits.size(); ++i) {
    for (int j = 0; j < circuits[i]->num_out_wires; ++j) {
      if (GetBitReversed(j, expected_output) != GetBit(j, outputs[i].get())) {
        // std::cout << "eval_num: " << i << " failed in out_bit: " << j << std::endl;
        all_success = false;
      }
    }
  }
  if (!all_success && (exec_name != "AES-CBC-MAC")) { //Do not have expected results for AES-CBC-MAC circuit so we skip the check
    std::cout << "Failure!" << std::endl;
    // return 0; //Do not return as we still report timings, even though result is wrong.
  }
  ...
  if (!print_special_format) {
    ...
    //Used for formatting output for latex paper
