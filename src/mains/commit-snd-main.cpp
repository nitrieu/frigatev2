...
int main(int argc, const char* argv[]) {
  ...
  std::unique_ptr<uint8_t[]> dot_base(std::make_unique<uint8_t[]>(params.num_OT * CSEC_BYTES));
  std::unique_ptr<uint8_t[]> dot_delta(std::make_unique<uint8_t[]>(CSEC_BYTES));
  ...
  uint8_t* global_delta = dot_delta.get();
  ...
  for (int i = 0; i < CODEWORD_BITS; ++i) {
    XOR_128(base_outer_delta, dot_base.get() + (rot_start_pos + i) * CSEC_BYTES, global_delta);
    params.crypt.hash(rot_seeds0.get() + i * CSEC_BYTES, CSEC_BYTES, dot_base.get() + (rot_start_pos + i) * CSEC_BYTES, CSEC_BYTES);
