...
public:
  ...
  EvalGarbledCircuit(Circuit& circuit, uint8_t tables);
  ...
  static uint64_t AuxDataSize(Circuit& circuit) {
    ...
    //key shares
    circuit.num_inp_wires * CODEWORD_BYTES +
    circuit.num_out_wires * CODEWORD_BYTES +
    CODEWORD_BYTES +
    //bit shares
    (circuit.num_inp_wires + circuit.num_out_wires) * BIT_CODEWORD_BYTES;
  };
  ...
  static uint8_t* inp_bit_share(Circuit& circuit, uint8_t* aux_info, uint32_t idx = 0) {
    return delta_share(circuit, aux_info) + CODEWORD_BYTES + idx * BIT_CODEWORD_BYTES;
  };

  static uint8_t* out_bit_share(Circuit& circuit, uint8_t* aux_info, uint32_t idx = 0) {
    return inp_bit_share(circuit, aux_info, circuit.num_inp_wires) + idx * BIT_CODEWORD_BYTES;
  };

  static uint8_t* inp_soldering(Circuit& circuit, uint8_t* aux_info, uint32_t idx = 0) {
    return out_bit_share(circuit, aux_info, circuit.num_out_wires) + idx * CSEC_BYTES;
  };
