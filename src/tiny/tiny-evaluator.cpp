...
TinyEvaluator::TinyEvaluator(Params& params) :
  Tiny(params),
  ot_rec(params),
  rot_seeds(std::make_unique<uint8_t[]>(NUM_COMMIT_SEED_OT * CSEC_BYTES + BITS_TO_BYTES(NUM_COMMIT_SEED_OT))),
  rot_choices(rot_seeds.get() + NUM_COMMIT_SEED_OT * CSEC_BYTES),
  dot_response(std::make_unique<uint8_t[]>(params.num_OT * CSEC_BYTES)),
  dot_choices(std::make_unique<uint8_t[]>(BITS_TO_BYTES(params.num_OT))),
...
void TinyEvaluator::Setup() {
  ...
  uint8_t commit_seed_OT_tmp[NUM_COMMIT_SEED_OT * CSEC_BYTES + BITS_TO_BYTES(NUM_COMMIT_SEED_OT)] = {0};
  uint8_t* commit_seed_OT_choices_tmp = commit_seed_OT_tmp + (NUM_COMMIT_SEED_OT) * CSEC_BYTES;

  ot_rec.Receive(NUM_COMMIT_SEED_OT, commit_seed_OT_tmp, commit_seed_OT_choices_tmp);

  //Requires computing commit_seed_OT_tmp0 \xor delta and then hashing both values into a rot to remove the correlation
  for (int i = 0; i < NUM_COMMIT_SEED_OT; ++i) {
    params.crypt.hash(commit_seed_OT_tmp + i * CSEC_BYTES, CSEC_BYTES, commit_seed_OT_tmp + i * CSEC_BYTES, CSEC_BYTES);
  }

  //Write data to file
  WriteFile(RECEIVER_COMMIT_SEED_OT, commit_seed_OT_tmp, NUM_COMMIT_SEED_OT * CSEC_BYTES + BITS_TO_BYTES(NUM_COMMIT_SEED_OT));
...
void TinyEvaluator::Preprocess() {
  ...
  //Load commit_seed_OT.
  ReadFile(RECEIVER_COMMIT_SEED_OT, rot_seeds.get(), NUM_COMMIT_SEED_OT * CSEC_BYTES + BITS_TO_BYTES(NUM_COMMIT_SEED_OT));

  auto dot_begin = GET_TIME();

  //Run DOT Extension.
  ot_rec.Receive(params.num_OT, dot_response.get(), dot_choices.get());
  ...
  params.rnd.GenRnd(bucket_seed, CSEC_BYTES);
  PRNG bucket_rnd;
  ...
  for (int exec_id = 0; exec_id < params.num_execs; ++exec_id) {
    ...
    commit_recs.emplace_back(std::make_unique<CommitReceiver>(*thread_params, rot_seeds.get(), rot_choices));
    ...
    cnc_execs_finished[exec_id] = thread_pool.push([this, thread_params, commit_rec, delta_holder, exec_id, &cout_mutex, &delta_checks, ver_success, inp_from, inp_to, last_exec_id, permuted_eval_gates_ids, permuted_eval_auths_ids, &durations] (int id) {
      ...
      if (!commit_rec->Commit()) {
        *ver_success = false;
        std::cout << "Commitment failed!" << std::endl;
      }
      ...
      if (exec_id == 0) {
        ...
        thread_params->chan.ReceiveBlocking(correction_commit_delta, CODEWORD_BYTES);
        for (int i = 0; i < CODEWORD_BYTES; ++i) {
          commit_rec->commit_shares[thread_params->delta_pos][i] ^= (correction_commit_delta[i] & REVERSE_BYTE_ORDER[commit_rec->choices[i]]);
        }
        ...
        unique_lock<mutex> lock(delta_received_mutex);
        while (!delta_received) {
          delta_received_cond_val.wait(lock);
        }

        commit_rec->commit_shares[thread_params->delta_pos] = delta_holder->commit_shares[delta_holder->params.delta_pos];
      }
      ...
      if (exec_id == last_exec_id) {
        ...
        std::copy(dot_response.get() + inp_to * CSEC_BYTES, dot_response.get() + inp_to * CSEC_BYTES + SSEC * CSEC_BYTES, cnc_ot_values.get());
        uint8_t* delta_ot_cnc_choices = cnc_ot_values.get() + SSEC * CSEC_BYTES;
        for (int i = 0; i < SSEC; ++i) {
          if (GetBit((inp_to + i), dot_choices.get())) {
        ...
        if (commit_rec->ChosenDecommit(chosen_computed_shares.get(), OT_decomitted_values.get(), chosen_decommit_pos, SSEC)) {
          for (int i = 0; i < SSEC; ++i) {
            uint8_t* own_ot = dot_response.get() + inp_to * CSEC_BYTES + i * CSEC_BYTES;
      ...
      for (int i = 0; i < thread_params->num_pre_inputs; ++i) {
        ...
        if ((GetLSB(dot_response.get() + (inp_from + i) * CSEC_BYTES) ^ GetBit(inp_from + i, dot_choices.get())) != (GetBit(thread_params->num_pre_outputs + i, local_verleak_bits))) {
      ...
      gh.ShiftEvaluateGates(gates_data, 0, all_cnc_keys.get() + num_check_auths * CSEC_BYTES, all_cnc_keys.get() + (num_check_auths + num_check_gates) * CSEC_BYTES, all_cnc_keys.get() + (num_check_auths + 2 * num_check_gates) * CSEC_BYTES, check_gate_ids.get(), num_check_gates, exec_id * (thread_params->Q + thread_params->A));
...
/////////////////// DEBUG for testing correctness of solderings//////////////
  ...
  for (int i = 0; i < params.num_pre_gates; ++i) {
    ...
    for (int j = 0; j < params.num_bucket; ++j) {
      ...
      gh.FullShiftEvaluateGates(eval_gates, curr_gate_pos, left_key, right_key, out_key, eval_gates_ids, 1);
    ...
    for (int j = 0; j < params.num_auth; ++j) {
      ...
      if (!gh.FullVerifyAuths(eval_auths, curr_auth_pos, out_key, eval_auths_ids, 1)) {
  ...
  for (int i = 0; i < params.num_pre_inputs; ++i) {
    ...
    for (int j = 0; j < params.num_inp_auth; ++j) {
      ...
      if (!gh.FullVerifyAuths(eval_auths, curr_auth_pos, auth_key, eval_auths_ids, 1)) {
    ...
    online_execs_finished[exec_id] = thread_pool.push([this, thread_params, exec_id, circ_from, circ_to, &circuits, &inputs, &outputs, &eval_gates_to_blocks, &eval_auths_to_blocks] (int id) {
      ...
      for (int c = circ_from; c < circ_to; ++c) {
        ...
        for (int i = 0; i < circuit->num_eval_inp_wires; ++i) {
          ...
          if (GetBit(curr_input, dot_choices.get())) {
            SetBit(i, 1, ot_input);
          } else {
            SetBit(i, 0, ot_input);
          }
        ...
        decommit_shares_out_1 = decommit_shares_out_0 + circuit->num_out_wires * CODEWORD_BYTES;
        if (!VerifyDecommits(decommit_shares_inp_0, decommit_shares_inp_1, eval_computed_shares_inp, eval_inp_keys, rot_choices, commit_recs[exec_id]->code.get(), circuit->num_eval_inp_wires)) {
        ...
        for (int i = 0; i < circuit->num_eval_inp_wires; ++i) {
          ...
          //XOR out K^i_y_i
          XOR_128(eval_inp_keys + i * CSEC_BYTES, dot_response.get() + curr_input * CSEC_BYTES);
...
/////////////////////////////// DEBUG Input buckets////////////////////////////
        ...
        if (!VerifyDecommits(decommit_shares_out_0, decommit_shares_out_1, eval_computed_shares_out, out_decommit_values, rot_choices, commit_recs[exec_id]->code.get(), circuit->num_out_wires)) {
          throw std::runtime_error("Abort: Wrong eval keys sent!");
        }
