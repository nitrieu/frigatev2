...
TinyConstructor::TinyConstructor(Params& params) :
  ...
  rot_seeds0(std::make_unique<uint8_t[]>(2 * NUM_COMMIT_SEED_OT * CSEC_BYTES)),
  rot_seeds1(rot_seeds0.get() + NUM_COMMIT_SEED_OT * CSEC_BYTES),
  dot_base(std::make_unique<uint8_t[]>(params.num_OT * CSEC_BYTES)),
  dot_delta(std::make_unique<uint8_t[]>(CSEC_BYTES)),
...
void TinyConstructor::Setup() {
  ...
  uint8_t commit_seed_OT_tmp0[2 * NUM_COMMIT_SEED_OT * CSEC_BYTES] = {0};
  uint8_t* commit_seed_OT_tmp1 = commit_seed_OT_tmp0 + NUM_COMMIT_SEED_OT * CSEC_BYTES;
  
  ot_snd.Send(NUM_COMMIT_SEED_OT, commit_seed_OT_tmp0, delta);
  ...
  for (int i = 0; i < NUM_COMMIT_SEED_OT; ++i) {
    XOR_128(commit_seed_OT_tmp1 + i * CSEC_BYTES, commit_seed_OT_tmp0 + i * CSEC_BYTES, delta);

    params.crypt.hash(commit_seed_OT_tmp0 + i * CSEC_BYTES, CSEC_BYTES, commit_seed_OT_tmp0 + i * CSEC_BYTES, CSEC_BYTES);
    params.crypt.hash(commit_seed_OT_tmp1 + i * CSEC_BYTES, CSEC_BYTES, commit_seed_OT_tmp1 + i * CSEC_BYTES, CSEC_BYTES);
  }

  //Write output to file
  WriteFile(SENDER_COMMIT_SEED_OT, commit_seed_OT_tmp0, 2 * NUM_COMMIT_SEED_OT * CSEC_BYTES);
...
void TinyConstructor::Preprocess() {
  ...
  //Load commit_seed_OTs.
  ReadFile(SENDER_COMMIT_SEED_OT, rot_seeds0.get(), 2 * NUM_COMMIT_SEED_OT * CSEC_BYTES);

  auto dot_begin = GET_TIME();
  
  //Run DOT Extension.
  ot_snd.Send(params.num_OT, dot_base.get(), dot_delta.get());
  //Pointer for global delta for convenience
  global_delta = dot_delta.get();
  ...
  for (int exec_id = 0; exec_id < params.num_execs; ++exec_id) {
    ...
    cnc_execs_finished[exec_id] = thread_pool.push([this, thread_params, commit_snd, delta_holder, exec_id, &cout_mutex, &delta_checks, inp_from, inp_to, last_exec_id, &durations, tmp_auth_eval_ids, tmp_gate_eval_ids] (int id) {
      ...
      uint8_t* inp_from_pointer = dot_base.get() + inp_from * CSEC_BYTES;
      ...
      if (exec_id == last_exec_id) {
        ...
        for (int i = 0; i < SSEC; ++i) {
          ...
          if (GetBit(i, cnc_ot_choices)) {
            ...
            XOR_128(correct_ot_value, dot_base.get() + (inp_to + i) * CSEC_BYTES, global_delta);
            ...
            std::copy(dot_base.get() + (inp_to + i) * CSEC_BYTES, dot_base.get() + (inp_to + i) * CSEC_BYTES + CSEC_BYTES, correct_ot_value);
          }
...
void TinyConstructor::BatchDecommitLSB(CommitSender* commit_snd, uint8_t decommit_shares0[], uint8_t decommit_shares1[], int num_values) {
  ...
  //Setup all registers for calculation the linear combinations. Will end up with SSEC linear combinations.
